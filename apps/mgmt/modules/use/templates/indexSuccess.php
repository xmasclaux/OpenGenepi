...
$(document).ready(function() {
	...
	*/
	...
	/*
	This function select automatically a list of users
	*/
	function directMultiSelect(usersId)
	{
		if ( usersId != null )
		{
			var allData = usersDatatable.fnGetData();
			...
			for (var i=0; i< usersId.length; i++)
			{
				if(usersId[i]!=null)
				{
					...
					{
						if ((allData[j]!=null) && (allData[j][4]==usersId[i]))
						{
						    /* Move only the row indicated by the parameter*/
						    var data=usersDatatable.fnGetData(j);
							data[0]="<div align=center><input  align=center type=\"checkbox\"></input></div>";
							usersDatatable.fnDeleteRow( j,null,true );
							usersToAddDatatable.fnAddData( data,true);
