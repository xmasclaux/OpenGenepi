...
class PDFExportDocument extends ExportDocument{
	
	public function __construct($locale){
		$this->setDocumentType(parent::PDF_FORMAT);
	...
	/**
 	...
 	protected function exportQuantitativeStatement($from, $to){
		...
		$this->savePDF('quantitave_statement');	
	...
	/**
 	...
 	protected function exportTemporalStatement($from, $to){
		...
		$this->savePDF('temporal_statement');	
 	 ...
 	 * 
 	 */
 	private function savePDF($ressource){
 		
 		$filename = 'export_'.$ressource.'_'.date('d-m-y').'.pdf';
 		
 		header('Content-Type: application/pdf; charset=utf-8');
		header('Content-Disposition: attachment;filename="'.$filename.'"');
		header('Cache-Control: max-age=0');
		
		$objWriter = PHPExcel_IOFactory::createWriter($this->getDocument(), 'PDF');
