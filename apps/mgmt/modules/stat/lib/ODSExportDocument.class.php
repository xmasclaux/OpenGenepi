...
class ODSExportDocument extends ExportDocument{
	...
	public function __construct($locale){
		$this->setDocumentType(parent::ODS_FORMAT);
	...
	/**
 	...
 	protected function exportQuantitativeStatement($from, $to){
		...
		$this->saveODS('quantitave_statement');	
	...
	/**
 	...
 	protected function exportTemporalStatement($from, $to){
		...
		$this->saveODS('temporal_statement');	
 	 ...
 	 * 
 	 */
 	private function saveODS($ressource){
 		
 		$filename = 'export_'.$ressource.'_'.date('d-m-y').'.ods';
